...
public:
    ...
    std::string dump(int indent = 2, int level = 0, bool root = false) const {
        ...
        if (isFloat()) {
            ...
            oss << std::fixed << std::setprecision(5) << asFloat();
        ...
        if (isArray()) {
            ...
            for (size_t i = 0; i < asArray().size(); ++i) {
                s += asArray()[i].dump(indent, level + 1);
                if (i + 1 < asArray().size()) s += ", ";
            }
            return s + "]";
        }
        if (isObject()) {
...
public:
    ...
    const Value* get(const std::string& path) const {
        if (!_root.isObject()) return nullptr;
...
struct Parser {
    ...
    void skip_ws() {
        while (pos < text.size()) {
            if (isspace((unsigned char)text[pos])) advance();
            else if (text[pos] == '#') {
                advance();
                if (peek() == '#') {
                    advance();
                    while (pos + 1 < text.size() && !(text[pos] == '#' && text[pos + 1] == '#')) advance();
                    if (pos < text.size()) { advance(); advance(); }
                } else {
    ...
    std::string read_key() {
        skip_ws(); std::string k;
        char c = peek();
        if (c == '"' || c == '\'') {
            char q = advance();
            while (peek() && peek() != q) {
                if (peek() == '\\') { advance(); char e = advance(); if (e == 'n') k += '\n'; else k += e; }
                else k += advance();
            }
            if (peek() == q) advance();
        } else {
    ...
    virtual Value parse_value() {
        skip_ws();
        if (std::chrono::steady_clock::now() - start > std::chrono::seconds(1)) throw std::runtime_error("Timeout");
        if (++depth > 100) throw std::runtime_error("Depth");
        char c = peek(); Value r;
        if (c == '{') {
            advance(); Object o;
            while (peek() && peek() != '}') {
                skip_ws(); if (peek() == '}') break;
                std::string k = read_key();
                if (k.empty()) break;
                if (k == "_env" || k == "_meta" || k == "_interpolate") throw std::runtime_error("Reserved");
            ...
            char q = advance(); std::string b;
            while (peek() && peek() != q) {
                if (peek() == '\\') { advance(); char e = advance(); if (e == 'n') b += '\n'; else b += e; }
                else b += advance();
            }
            if (peek() == q) advance();
            r = std::move(b);
            ...
            while (true) {
                char ch = peek();
                if (ch && (isdigit((unsigned char)ch) || strchr(".-eE+", ch))) b += advance();
                else break;
            }
            if (b.find('.') != std::string::npos || b.find('e') != std::string::npos || b.find('E') != std::string::npos) r = std::stod(b);
            else r = (int64_t)std::stoll(b);
        } else {
            std::string b;
            while (isalpha((unsigned char)peek())) b += advance();
            if (b == "true") r = true;
            else if (b == "false") r = false;
            else if (b == "null") r = nullptr;
        }
        depth--; return r;
    }
    virtual Object parse() {
        ...
        while (pos < text.size()) {
            ...
            if (c == '[') {
                advance(); std::string path;
                while (peek() && peek() != ']') path += advance();
                if (peek() == ']') advance();
    ...
    virtual void parse_into(Object& o) {
        while (pos < text.size()) {
            skip_ws(); char c = peek(); if (c == '[' || !c) break;
            std::string k = read_key();
            if (k.empty()) break;
            skip_ws(); if (peek() == ':') advance();
            o[k] = parse_value();
...
inline Document parse(std::string text) { return Document(Value(Parser(std::move(text)).parse())); }
}
#endif
