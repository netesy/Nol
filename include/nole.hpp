...
/*
    NOLE - Notation Object Language Extended
    ...
    Phase 1 — NOLE.hpp Core Complete (real code)
    full lexer
    full parser
    sections
    arrays
...
enum class Mode {
    Core,       // Default - no evaluation features
    Extended    // Opt-in - allows evaluation features
};
...
public:
    ...
    Token next() {
        ...
        switch (c) {
            ...
            case '$': get(); return token(Tok::Dollar, "$");
            case '<': {
                get();
                if (!eof() && peek() == '<') {
                    get();
                    return token(Tok::LShift, "<<");
                }
                return token(Tok::LAngle, "<");
            }
            case '>': get(); return token(Tok::RAngle, ">");
            case '&': get(); return token(Tok::Ampersand, "&");
...
private:
    ...
    Token number() {
        ...
        // Check if this could be a date pattern (YYYY-MM-DD)
        if (!dot && s.length() >= 4 && !eof() && peek() == '-') {
            // Potential date, collect the full pattern
            s += get(); // add the hyphen
            ...
            if (!eof() && peek() == '-') {
                ...
                // Check if this could be a datetime pattern (YYYY-MM-DDTHH:MM:SSZ)
                if (s.length() == 10 && !eof() && peek() == 'T') {
                    s += get(); // add the T
                    ...
                    if (!eof() && peek() == ':') {
                        ...
                        if (!eof() && peek() == ':') {
                            ...
                            if (!eof() && peek() == 'Z') {
                                ...
                                // Validate datetime format YYYY-MM-DDTHH:MM:SSZ
                                if (s.length() == 20 && s[4] == '-' && s[7] == '-' && s[10] == 'T' && s[13] == ':' && s[16] == ':' && s[19] == 'Z') {
                                    bool valid_datetime = true;
                                    // Check date part
                                    for (int i = 0; i < 10; i++) {
                                        if (i == 4 || i == 7) continue;
...
public:
    ...
    Value parse() {
        ...
        while (cur.type != Tok::End) {
            ...
            else if (cur.type == Tok::LBrace || cur.type == Tok::LBracket || cur.type == Tok::Dollar || cur.type == Tok::LAngle) {
                // Handle standalone values (not String since it's handled above)
                return parseValue();
            }
        ...
        // Post-process interpolations
        resolveInterpolations(root);
        return root;
    }
...
private:
    ...
    void checkReservedKey(const std::string& key) {
        // Reserved key names in NOLE
        if (key == "_env" || key == "_interpolate" || key == "_meta") {
            error("Reserved key name not allowed: " + key);
        }
    }
    ...
    Value parseValue() {
        ...
        switch (cur.type) {
            ...
            case Tok::Ampersand:
                // Handle anchor definition at value level
                return parseAnchorDefinition();

            case Tok::LShift:
                // Handle merge at value level  
                return parseMerge();
    ...
    Value parseObject() {
        ...
        while (cur.type != Tok::RBrace) {
            ...
            // Handle anchor definition: &anchor_name { ... }
            if (cur.type == Tok::Ampersand) {
                next();
                if (cur.type != Tok::Identifier)
                    error("Expected anchor name after &");
                
                std::string anchorName = cur.text;
                next();
                
                Value anchoredValue = parseValue();
                anchors_.emplace(anchorName, anchoredValue);
                
                // If it's an object, merge it into current object
                if (anchoredValue.isObject()) {
                    for (const auto& kv : anchoredValue.asObject()) {
                        if (obj.find(kv.first) == obj.end()) {
            ...
            if (cur.type == Tok::LShift) {
                ...
                auto it = anchors_.find(anchorName);
                if (it == anchors_.end()) {
                    error("Anchor not found: " + anchorName);
                }
                
                // Merge the anchored object
                if (it->second.isObject()) {
                    for (const auto& kv : it->second.asObject()) {
                        if (obj.find(kv.first) == obj.end()) {
                            obj[kv.first] = kv.second;
                        }
                    }
    ...
    Value parseEnvVar() {
        ...
        if (cur.type != Tok::Identifier)
            error("Expected environment variable name after $");
        ...
        if (cur.type == Tok::QuestionQuestion) {
            next();
            fallback = parseValue();
            hasFallback = true;
        }
        ...
        if (envValue) {
            ...
            error("Environment variable not set: " + varName);
        }
    }

    Value parseCoercedValue() {
        expect(Tok::LAngle);
        next();
        ...
        if (typeName == "int") {
            if (value.isString()) {
                try {
                    return (int64_t)std::stoll(value.asString());
                } catch (const std::exception&) {
                    error("Cannot coerce to int: " + value.asString());
                }
            } else if (value.isInt()) {
                return value;
            ...
            if (value.isString()) {
                try {
                    ...
                    error("Cannot coerce to float: " + value.asString());
                }
            } else if (value.isFloat()) {
                return value;
            } else if (value.isInt()) {
        ...
        error("Invalid coercion type: " + typeName);
    }

    void resolveInterpolations(Value& v) {
        if (v.isString()) {
            ...
            while (pos < s.length()) {
                size_t dollarPos = s.find("${", pos);
                if (dollarPos == std::string::npos) {
                    result += s.substr(pos);
                    break;
                }
                
                result += s.substr(pos, dollarPos - pos);
                
                size_t closePos = s.find('}', dollarPos + 2);
                if (closePos == std::string::npos) {
                    error("Unclosed interpolation");
                }
                
                std::string path = s.substr(dollarPos + 2, closePos - dollarPos - 2);
                Value* ref = resolveReference(*root_, path);
                if (!ref) {
            ...
            for (auto& item : v.asArray()) {
                resolveInterpolations(item);
            }
        } else if (v.isObject()) {
            for (auto& kv : v.asObject()) {
                resolveInterpolations(kv.second);
            }
        }
    }
    
    Value* resolveReference(Value& root, const std::string& path) {
    ...
    Value parseAnchorDefinition() {
        ...
        // Store the anchor
        anchors_.emplace(anchorName, value);
...
// Legacy API - Exception-based parsing (keep for compatibility)
inline Value parseOrThrow(const std::string& content) {
...
inline bool writeFile(const std::string& path, const Value& v) {
    ...
    if (!out) return false;
    out << stringify(v);
    return out.good(); // Changed return true to return out.good()
}
